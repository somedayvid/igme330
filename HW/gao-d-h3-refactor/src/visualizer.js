...
import Arrow from './NoteSprite.js';
  ...
  // 1 - populate the audioData array with the frequency data from the analyserNode
	...
	for(let index in arrowsList){
		for(let furtherIndex in arrowsList[index]){
			...
			if(params.showNotes){
				arrowsList[index][furtherIndex].draw(ctx);
			}
		}
	}
...
const addToHitQualityList = (hit) =>{
	hitQualityList.push(hit);
}


export {setupCanvas,draw, clearAll, addToHitQualityList, canvasStartX, canvasHeight, continueCombo};